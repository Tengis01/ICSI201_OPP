...
public:
  ...
  void show_data() {
    ...
    if (strcmp(name,"triangle")==0){
      for(int i=0;i<3;i++) cout << "x"<<i<<": "<<xy[i][0]<<" y"<<i<<": "<<xy[i][1]<<"\n";
    }
    else if(strcmp(name,"square")==0){
      for(int i=0;i<4;i++) cout << "x"<<i<<": "<<xy[i][0]<<" y"<<i<<": "<<xy[i][1]<<"\n";
    }
    cout << "side/radius: " << a << "\n";
    cout << "area: " << area() << "\n";
...
public:
  ...
  void coordinate() override {} // circle-д координат хэрэггүй
};
// Square class
...
public:
  ...
  float area() override { float h=sqrt(a*a-(a/2)*(a/2)); return a*h/2; }
  float perimeter() override { return 3*a; }
  void coordinate() override {
    float h=sqrt(a*a-(a/2)*(a/2));
    xy[1][0]=xy[0][0]+a/2; xy[1][1]=xy[0][1]-h;
    xy[2][0]=xy[0][0]-a/2; xy[2][1]=xy[0][1]-h;}};            template<typename T>
class GeneralList {
private:
...
public:
 ...
 shapes.add(&c1); shapes.add(&c2); shapes.add(&c3); shapes.add(&c4); shapes.add(&c5); shapes.add(&c6); shapes.add(&c7); shapes.add(&c8); shapes.add(&c9);
 shapes.add(&s1); shapes.add(&s2); shapes.add(&s3); shapes.add(&s4); shapes.add(&s5); shapes.add(&s6); shapes.add(&s7); shapes.add(&s8); shapes.add(&s9);
 shapes.add(&t1); shapes.add(&t2); shapes.add(&t3); shapes.add(&t4); shapes.add(&t5); shapes.add(&t6); shapes.add(&t7); shapes.add(&t8); shapes.add(&t9);                       // Санамсаргүйгийн оронд гараар тал/радиус оноох
 ...
 cout << "\n== Жагсаалтын дунд оруулах / устгах туршилт ==\n";
 circle c10; c10.a=7;
